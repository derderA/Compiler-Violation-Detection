...
- `ADMIN_PASSWORD`：可选，首次启动时管理员密码的初始值；之后密码保存在 `data/db.json` 中，修改该变量不会覆盖已保存的密码
...
- **生成报告与摘要值**：任何人都可以操作，无需登录。
- **验证文件是否被篡改**：仅管理员可以操作。进入「管理员验证」区域需要先输入管理员密码，登录后：
  1. 上传原始文件（例如下载的 `analysis.md`）
  2. 输入文件处理后的摘要值（例如 `hash.txt` 中的十六进制字符串）
  3. 点击「开始验证」，系统会重新计算 HMAC-SHA256 摘要并比对：
     - 一致 → 文件未被篡改
     - 不一致 → 文件已被篡改
- **修改密码**：管理员登录后可在同一界面修改密码，修改后需重新登录。

管理员密码保存在 `data/db.json` 中，以 `scrypt` 加盐哈希存储，不保存明文。
